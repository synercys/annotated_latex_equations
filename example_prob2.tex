\begin{figure}[htb]
    ...
    \begin{tikzpicture}[overlay,remember picture,>=stealth,nodes={align=left,inner ysep=1pt},<-]
        % Ts to Td
        \path (ts.north) ++ (-1.5em,2em) node[anchor=south west,color=red!67] (ttext){\textbf{$\tau_i,\tau' \in \Gamma$}, \textbf{the set of Tasks}};
        \draw[<->,color=red!57] (ts.north) |- (ttext.south) -| (td.north);
        % js to jd
        \path (js.south) ++ (3em,-3.3em) node[anchor=south west,color=blue!67] (jtext){\textbf{$j,j'\in \mathbb{N}$}};
